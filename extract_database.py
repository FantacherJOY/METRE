...
def extract_mimic(args):
    ...
    cardiac_marker['troponin_t'] = cardiac_marker['troponin_t'].replace(to_replace=[None], value=np.nan)
    ...
    chart_lab = chart_lab.groupby(ID_COLS + group_item_cols + ['hours_in'])['value'].agg(['mean', 'count'])
    ...
    vital_encode = pd.get_dummies(vital).copy()
    ...
    # impute
    vital_final.loc[:, mean_col] = vital_final.loc[:, mean_col].groupby(ID_COLS).ffill().groupby(
        ID_COLS).fillna(
    ...
    # --- START HDF5 FIX ---
    # Convert all dataframe indexes to int64 to ensure HDF5 compatibility
    print("Converting index dtypes for HDF5 compatibility...")
    hdf_dfs = [
        vital_train, vital_dev, vital_test,
        Y_train, Y_dev, Y_test,
        static_train, static_dev, static_test
    ]
    for df in hdf_dfs:
        new_levels = [level.astype('int64') for level in df.index.levels]
